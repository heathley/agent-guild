...
if (!source.includes("guild_request_public_action") || source.includes('registerTool("post_message"')) throw new Error("Connector tool surface is unsafe or incomplete.");
...
try {
  ...
  if (names.length !== 8 || names.includes("post_message")) throw new Error(`Unexpected packaged MCP tools: ${names.join(", ")}`);
...
process.stdout.write("Connector package smoke passed: built entry starts, exposes 8 approval-gated tools, and answers guild_status.\n");
