...
if (!source.includes("guild_request_public_action") || !source.includes("guild_suggest_work") || source.includes('registerTool("post_message"')) throw new Error("Connector tool surface is unsafe or incomplete.");
...
try {
  ...
  if (names.length !== 9 || names.includes("post_message")) throw new Error(`Unexpected packaged MCP tools: ${names.join(", ")}`);
...
process.stdout.write("Connector package smoke passed: built entry starts, exposes 9 approval-gated tools, and answers guild_status.\n");
